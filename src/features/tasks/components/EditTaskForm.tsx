...
function EditTaskForm({ onCancel, projectOptions, memberOptions, initialValue }: EditTaskFormProps) {
  const { mutate, isPending } = useUpdateTask();
  const form = useForm<z.infer<typeof createTaskSchema>>({
    resolver: zodResolver(createTaskSchema.omit({ workspaceId: true, description: true })),
    defaultValues: { ...initialValue, dueDate: initialValue.dueDate ? new Date(initialValue.dueDate) : undefined },
  });

  const onSubmit = (data: z.infer<typeof createTaskSchema>) => {
    mutate(
      { json: data, param: { taskId: initialValue.$id } },
      {
        onSuccess: () => {
          form.reset();
          onCancel?.();
  ...
  return (
    <Card className="h-full w-full border-none shadow-none">
      <CardHeader className="flex p-7">
        <CardTitle className="text-xl font-bold">ویرایش تسک</CardTitle>
      </CardHeader>
      <div className="px-7">
        <DottedSeparator />
      </div>
